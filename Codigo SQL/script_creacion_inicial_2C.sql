...
CREATE TABLE [LA_REVANCHA].[HOTEL_CERRADO](
			 ...
			 [HOTCERR_CANT_DIAS_CERRADO][NUMERIC](18,0),
...
GO
CREATE TABLE [LA_REVANCHA].[ESTADIA](
			 [EST_COD_ESTADIA][NUMERIC](18,0) IDENTITY(250,3),
			 [EST_COD_RESERVA][NUMERIC](18,0),
			 [EST_FECHA_CHECK_IN][DATETIME],
			 [EST_FECHA_CHECK_OUT][DATETIME],
			 [EST_PRECIO][NUMERIC](18,2),
			 [EST_DIAS_ALOJADOS][NUMERIC](18,0),
			 [EST_DIAS_NO_ALOJADOS][NUMERIC](18,0),
PRIMARY KEY CLUSTERED (EST_COD_ESTADIA),
FOREIGN KEY (EST_COD_RESERVA) REFERENCES LA_REVANCHA.RESERVA(RES_CODIGO)
)
...
CREATE TABLE [LA_REVANCHA].[FACTURA](
			 ...
			 [FAC_COD_ESTADIA][NUMERIC](18,0),
			 [FAC_CONSUMIDOR][NUMERIC](18,0),	 
PRIMARY KEY CLUSTERED (FAC_CODIGO),
FOREIGN KEY (FAC_COD_FORMA_PAGO) REFERENCES LA_REVANCHA.FORMA_PAGO(PAGO_CODIGO),
FOREIGN KEY (FAC_COD_ESTADIA) REFERENCES LA_REVANCHA.ESTADIA(EST_COD_ESTADIA),
FOREIGN KEY (FAC_CONSUMIDOR) REFERENCES LA_REVANCHA.USUARIO(USU_CODIGO)
)
...
GO
CREATE TABLE [LA_REVANCHA].[ESTADIA_CONSUMIBLE_ITEM](
			 [ESCOIT_CODIGO][NUMERIC](18,0) IDENTITY (5000,1),
			 [ESCOIT_COD_ESTADIA][NUMERIC](18,0),
			 [ESCOIT_COD_CONSUMIBLE][NUMERIC](18,0),
			 [ESCOIT_COD_ITEM][NUMERIC](18,0),
			 [ESCOIT_COD_HABITACION][NUMERIC](18,0),
			 [ESCOIT_CANTIDAD][NUMERIC](18,0),
PRIMARY KEY CLUSTERED (ESCOIT_CODIGO),
FOREIGN KEY (ESCOIT_COD_ESTADIA) REFERENCES LA_REVANCHA.ESTADIA(EST_COD_ESTADIA),
FOREIGN KEY (ESCOIT_COD_CONSUMIBLE) REFERENCES LA_REVANCHA.CONSUMBILES(CONS_CODIGO),
FOREIGN KEY (ESCOIT_COD_ITEM) REFERENCES LA_REVANCHA.FACTURA_ITEM(FACITEM_CODIGO),
FOREIGN KEY (ESCOIT_COD_HABITACION) REFERENCES LA_REVANCHA.HABITACION(HAB_CODIGO)
)
...
UPDATE LA_REVANCHA.TIPO_HABITACION SET TIPHAB_CANT_PERSONAS = 1
WHERE TIPHAB_CODIGO = 1001

UPDATE LA_REVANCHA.TIPO_HABITACION SET TIPHAB_CANT_PERSONAS = 2
WHERE TIPHAB_CODIGO = 1002

UPDATE LA_REVANCHA.TIPO_HABITACION SET TIPHAB_CANT_PERSONAS = 3
WHERE TIPHAB_CODIGO = 1003

UPDATE LA_REVANCHA.TIPO_HABITACION SET TIPHAB_CANT_PERSONAS = 4
WHERE TIPHAB_CODIGO = 1004

UPDATE LA_REVANCHA.TIPO_HABITACION SET TIPHAB_CANT_PERSONAS = 5
WHERE TIPHAB_CODIGO = 1005
...
INSERT INTO LA_REVANCHA.RESERVA(RES_CODIGO,RES_FECHA_DESDE, RES_FECHA_HASTA, RES_HOTREG_HOTEL, RES_HOTREG_REGIMEN,RES_FECHA_CARGA)
...
H.HOT_CODIGO, R.REG_CODIGO, '2001-01-01 00:00:00.000'
...
INSERT INTO LA_REVANCHA.ESTADIA(EST_COD_RESERVA,EST_PRECIO)
SELECT DISTINCT Reserva_Codigo, Item_Factura_Monto
FROM gd_esquema.Maestra
WHERE Consumible_Codigo IS NULL AND Item_Factura_Monto IS NOT NULL

--TABLA FACTURA(89603)

INSERT INTO LA_REVANCHA.FACTURA(FAC_CODIGO,FAC_FECHA,FAC_TOTAL,FAC_COD_ESTADIA, FAC_CONSUMIDOR)
SELECT DISTINCT M.Factura_Nro, M.Factura_Fecha, M.Factura_Total, E.EST_COD_ESTADIA, C.CLI_COD_USUARIO
FROM gd_esquema.Maestra M JOIN LA_REVANCHA.CLIENTE C ON  
C.CLI_NUMERO_IDENTIFICACION=Cliente_Pasaporte_Nro AND
C.CLI_APELLIDO=Cliente_Apellido AND
C.CLI_NOMBRE=Cliente_Nombre JOIN LA_REVANCHA.ESTADIA E ON 
E.EST_COD_RESERVA=M.Reserva_Codigo
WHERE M.Factura_Nro IS NOT NULL


--TABLA FACTURA_ITEM 207341

INSERT INTO LA_REVANCHA.FACTURA_ITEM(FACITEM_COD_FACTURA, FACITEM_CANTIDAD, FACITEM_MONTO, FACITEM_HABITACION)
SELECT DISTINCT M.Factura_Nro,M.Item_Factura_Cantidad, M.Item_Factura_Monto, HA.HAB_CODIGO
FROM gd_esquema.Maestra M JOIN LA_REVANCHA.HOTEL H ON H.HOT_CALLE=Hotel_Calle AND H.HOT_NRO_CALLE=Hotel_Nro_Calle
 AND H.HOT_CIUDAD=Hotel_Ciudad  JOIN LA_REVANCHA.HABITACION HA ON HA.HAB_NUMERO=Habitacion_Numero AND 
 HA.HAB_COD_HOTEL=H.HOT_CODIGO
WHERE M.Factura_Nro IS NOT NULL AND M.Consumible_Codigo IS NOT NULL
ORDER BY M.Factura_Nro

--TABLA ESTADIA_CONSUMIBLE_ITEM 207341

INSERT INTO LA_REVANCHA.ESTADIA_CONSUMIBLE_ITEM(ESCOIT_COD_ESTADIA,ESCOIT_COD_CONSUMIBLE, ESCOIT_COD_HABITACION, ESCOIT_CANTIDAD,ESCOIT_COD_ITEM)
SELECT DISTINCT E.EST_COD_ESTADIA , M.Consumible_Codigo, HA.HAB_CODIGO, M.Item_Factura_Cantidad, FI.FACITEM_CODIGO
FROM gd_esquema.Maestra M JOIN LA_REVANCHA.HOTEL H ON H.HOT_CALLE=Hotel_Calle AND H.HOT_NRO_CALLE=Hotel_Nro_Calle
 AND H.HOT_CIUDAD=Hotel_Ciudad  JOIN LA_REVANCHA.HABITACION HA ON HA.HAB_NUMERO=Habitacion_Numero AND 
 HA.HAB_COD_HOTEL=H.HOT_CODIGO JOIN LA_REVANCHA.FACTURA_ITEM FI ON
 FI.FACITEM_HABITACION=HA.HAB_CODIGO AND FI.FACITEM_COD_FACTURA=M.Factura_Nro AND FI.FACITEM_CANTIDAD=M.Item_Factura_Cantidad
AND FI.FACITEM_MONTO=M.Item_Factura_Monto JOIN 
 LA_REVANCHA.ESTADIA E ON E.EST_COD_RESERVA=M.Reserva_Codigo 
WHERE M.Factura_Nro IS NOT NULL AND M.Consumible_Codigo IS NOT NULL
ORDER BY E.EST_COD_ESTADIA
...
--TRIGGER QUE CALCULA LA CANTIDAD DE DIAS QUE UN HOTEL ESTUVO CERRADO
	
GO
CREATE TRIGGER LA_REVANCHA.TR_CALCULADOR_DE_DIAS
ON LA_REVANCHA.HOTEL_CERRADO
FOR INSERT 
AS
	DECLARE @INICIO DATETIME
	DECLARE @FIN DATETIME
	
	SELECT @INICIO = HOTEL_CERRADO.HOTCERR_FECHA_DESDE  FROM LA_REVANCHA.HOTEL_CERRADO
										  JOIN inserted ON HOTEL_CERRADO.HOTCERR_COD_HOTEL = inserted.HOTCERR_COD_HOTEL      
										  WHERE inserted.HOTCERR_COD_HOTEL = HOTEL_CERRADO.HOTCERR_COD_HOTEL
			 
	
	SELECT @FIN = HOTEL_CERRADO.HOTCERR_FECHA_HASTA  FROM LA_REVANCHA.HOTEL_CERRADO
									   JOIN inserted ON HOTEL_CERRADO.HOTCERR_COD_HOTEL = inserted.HOTCERR_COD_HOTEL      
									   WHERE inserted.HOTCERR_COD_HOTEL = HOTEL_CERRADO.HOTCERR_COD_HOTEL
	...
	UPDATE LA_REVANCHA.HOTEL_CERRADO SET HOTEL_CERRADO.HOTCERR_CANT_DIAS_CERRADO =  DATEDIFF(DAY, @INICIO, @FIN)
																	  FROM LA_REVANCHA.HOTEL_CERRADO
																	  JOIN inserted ON inserted.HOTCERR_COD_HOTEL = HOTEL_CERRADO.HOTCERR_COD_HOTEL
																	  WHERE inserted.HOTCERR_COD_HOTEL = HOTEL_CERRADO.HOTCERR_COD_HOTEL
...
WHERE CANC_FECHA >= '20051231 08:00:00 AM' AND 	CANC_FECHA <= '20051231 08:00:00 AM'							
-----------------------------------------------------------------------------------------------------
PUNTO 2


SELECT TOP 5 SUM(FACITEM_CANTIDAD) AS CANTIDAD_CONSUMIBLES_POR_FACTURA,HOTEL.*
FROM LA_REVANCHA.HOTEL
JOIN LA_REVANCHA.HABITACION ON HOTEL.HOT_CODIGO = HABITACION.HAB_COD_HOTEL
JOIN LA_REVANCHA.HABITACION_RESERVA ON HABITACION.HAB_CODIGO = HABITACION_RESERVA.HABRES_COD_HABITACION
JOIN LA_REVANCHA.RESERVA ON HABITACION_RESERVA.HABRES_COD_RESERVA = RESERVA.RES_CODIGO
...
WHERE FAC_FECHA >= '20051231 08:00:00 AM' AND FAC_FECHA	<= '20051231 08:00:00 AM'
...
PUNTO 3
	...
	IR INSERTANDO DE UN REGISTRO POR VEZ PARA PROBAR.....
	
	
		INSERT INTO LA_REVANCHA.HOTEL_CERRADO(HOTCERR_COD_HOTEL,HOTCERR_FECHA_DESDE,HOTCERR_FECHA_HASTA)
		VALUES(1400, '20051231 08:00:00 AM', '20060101 08:00:00 AM'),	
								

	SELECT TOP 5 SUM(HOTEL_CERRADO.HOTCERR_CANT_DIAS_CERRADO) AS CANTIDAD_DE_DIAS_CERRADO, HOTEL_CERRADO.HOTCERR_COD_HOTEL,HOT_NOMBRE,HOT_MAIL,HOT_TELEFONO,HOT_CALLE,HOT_NRO_CALLE,
	             HOT_ESTRELLAS,HOT_RECARGA_ESTRELLAS,HOT_CIUDAD,HOT_PAIS,HOT_FECHA_CREACION,HOT_HABILITADO
	FROM LA_REVANCHA.HOTEL_CERRADO  
	JOIN LA_REVANCHA.HOTEL ON HOTCERR_COD_HOTEL = HOTEL.HOT_CODIGO
	WHERE LA_REVANCHA.HOTEL_CERRADO.HOTCERR_FECHA_DESDE = '20051231 08:00:00 AM'  AND LA_REVANCHA.HOTEL_CERRADO.HOTCERR_FECHA_HASTA = '20060131 08:00:00 AM'
	GROUP BY HOTEL_CERRADO.HOTCERR_COD_HOTEL, HOTEL_CERRADO.HOTCERR_COD_HOTEL, HOT_NOMBRE,HOT_MAIL,HOT_TELEFONO,HOT_CALLE,HOT_NRO_CALLE,
		     HOT_ESTRELLAS,HOT_RECARGA_ESTRELLAS,HOT_CIUDAD,HOT_PAIS,HOT_FECHA_CREACION,HOT_HABILITADO
	ORDER BY CANTIDAD_DE_DIAS_CERRADO DESC


-----------------------------------------------------------------------------------------------------
PUNTO 4.

	SELECT TOP 5 HABITACION.*,SUM(ESTADIA.EST_DIAS_ALOJADOS) AS DIAS_HABITACION_ALOJADOS, COUNT(HABITACION_RESERVA.HABRES_COD_HABITACION) AS CANTIDAD_VECES_OCUPADA
	FROM LA_REVANCHA.HOTEL
	JOIN LA_REVANCHA.HABITACION ON HOTEL.HOT_CODIGO = HABITACION.HAB_COD_HOTEL
	JOIN LA_REVANCHA.HABITACION_RESERVA ON HABITACION.HAB_CODIGO = HABITACION_RESERVA.HABRES_COD_HABITACION
	JOIN LA_REVANCHA.RESERVA ON HABITACION_RESERVA.HABRES_COD_RESERVA = RESERVA.RES_CODIGO
	JOIN LA_REVANCHA.ESTADIA ON RESERVA.RES_CODIGO = ESTADIA.EST_COD_RESERVA
	WHERE RESERVA.RES_ESTRES_CODIGO <> 4002 AND RESERVA.RES_ESTRES_CODIGO <> 4003 AND RESERVA.RES_FECHA_DESDE >= '20051231 08:00:00 AM' AND RESERVA.RES_FECHA_HASTA <= '20051231 08:00:00 AM'
...
SELECT DISTINCT TOP 5 CLIENTE.*,(((ESTADIA.EST_PRECIO*TIPO_HABITACION.TIPHAB_CANT_PERSONAS*HOTEL.HOT_ESTRELLAS)/10) + FACTURA.FAC_TOTAL/5) AS PUNTOS_TOTALES
...
WHERE REGIMEN.REG_CODIGO <> 120 AND REGIMEN.REG_CODIGO <> 130 AND FAC_FECHA >= '20051231 08:00:00 AM' AND FAC_FECHA	<= '20051231 08:00:00 AM'
