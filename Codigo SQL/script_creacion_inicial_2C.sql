...
CREATE TABLE [LA_REVANCHA].[FACTURA_ITEM](
			 ...
			 [FACITEM_CONSUMIDOR][NUMERIC](18,0),
...
FOREIGN KEY (FACITEM_CONSUMIDOR) REFERENCES LA_REVANCHA.USUARIO(USU_CODIGO)
)
...
INSERT INTO LA_REVANCHA.FACTURA_ITEM(FACITEM_COD_FACTURA,FACITEM_COD_CONSUMIBLE,FACITEM_CANTIDAD,FACITEM_MONTO, FACITEM_CONSUMIDOR)
SELECT DISTINCT M.Factura_Nro, M.Consumible_Codigo, M.Item_Factura_Cantidad, M.Item_Factura_Monto, C.CLI_COD_USUARIO
FROM gd_esquema.Maestra M LEFT JOIN LA_REVANCHA.CLIENTE C ON  
C.CLI_NUMERO_IDENTIFICACION=Cliente_Pasaporte_Nro AND
C.CLI_APELLIDO=Cliente_Apellido AND
C.CLI_NOMBRE=Cliente_Nombre
