...
PUNTO 4
	...
	SELECT TOP 5 HABITACION.HAB_CODIGO,HABITACION.HAB_NUMERO,SUM(FACTURA.FAC_DIAS_ALOJADOS) AS DIAS_HABITACION_ALOJADOS, COUNT(HABITACION_RESERVA.HABRES_COD_HABITACION) AS CANTIDAD_VECES_OCUPADA,HOTEL.*
	FROM LA_REVANCHA.HOTEL
	JOIN LA_REVANCHA.HABITACION ON HOTEL.HOT_CODIGO = HABITACION.HAB_COD_HOTEL
	JOIN LA_REVANCHA.HABITACION_RESERVA ON HABITACION.HAB_CODIGO = HABITACION_RESERVA.HABRES_COD_HABITACION
	JOIN LA_REVANCHA.RESERVA ON HABITACION_RESERVA.HABRES_COD_RESERVA = RESERVA.RES_CODIGO
	...
	GROUP BY HABITACION.HAB_CODIGO, HOT_CODIGO,HOT_NOMBRE,HOT_MAIL,HOT_TELEFONO,HOT_CALLE,HOT_NRO_CALLE,
		     HOT_ESTRELLAS,HOT_RECARGA_ESTRELLAS,HOT_CIUDAD,HOT_PAIS,HOT_FECHA_CREACION,HOT_HABILITADO, HABITACION.HAB_NUMERO
	ORDER BY DIAS_HABITACION_ALOJADOS DESC, CANTIDAD_VECES_OCUPADA DESC
----------------------------------------------------------------------------------------------------------------------
PUNTO 5

********************************************************************************************************************************************************************
OBTENGO LA SUMATORIA DE LOS PUNTOS EN TOTAL (PUNTOS_ESTADIA + PUNTOS_CONSUMIBLES), EL PROBLEMA QUE ESTOY TENIENDO ES LA SUMA NO COINCIDE QUE CALCULANDO POR SEPARADO
********************************************************************************************************************************************************************

SELECT DISTINCT CLIENTE.CLI_COD_USUARIO,(SUM((ESTADIA.EST_PRECIO*TIPO_HABITACION.TIPHAB_CANT_PERSONAS*HOTEL.HOT_ESTRELLAS)/10) + (FACTURA.FAC_TOTAL/5) )
...
WHERE REGIMEN.REG_CODIGO <> 120 AND REGIMEN.REG_CODIGO <> 130
GROUP BY CLIENTE.CLI_COD_USUARIO, ESTADIA.EST_PRECIO, TIPO_HABITACION.TIPHAB_CANT_PERSONAS, HOTEL.HOT_ESTRELLAS, FACTURA.FAC_TOTAL
ORDER BY CLIENTE.CLI_COD_USUARIO ASC


********************************************************************************************************************************
OBTENGO LOS PUNTOS_ESTADIA Y PUNTOS_CONSUMIBLES POR SEPARADO. SI LOS SUMO ME DA DISTINTO A LA QUERY DE ARRIBA.
********************************************************************************************************************************

SELECT DISTINCT CLIENTE.CLI_COD_USUARIO, (ESTADIA.EST_PRECIO*TIPO_HABITACION.TIPHAB_CANT_PERSONAS*HOTEL.HOT_ESTRELLAS)/10 AS PUNTOS_ESTADIA, FACTURA.FAC_TOTAL/5 AS PUNTOS_CONSUMIBLES
FROM LA_REVANCHA.REGIMEN,LA_REVANCHA.CLIENTE
JOIN LA_REVANCHA.USUARIO ON CLIENTE.CLI_COD_USUARIO = USUARIO.USU_CODIGO
JOIN LA_REVANCHA.RESERVA_USUARIO ON USUARIO.USU_CODIGO = RESERVA_USUARIO.RESUSU_CODUSU_HUESPED
JOIN LA_REVANCHA.RESERVA ON RESERVA_USUARIO.RESUSU_COD_RESERVA = RESERVA.RES_CODIGO
